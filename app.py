...
def handle_error(error: Exception, status_code: int = 500) -> tuple:
    ...
    return get_response(success=False, message=str(error), status_code=status_code)


@app.route('/api/health', methods=['GET'])
def health_check() -> tuple:
    """Health check endpoint."""
    return get_response(success=True, message='Backend is healthy',
...
def get_config() -> tuple:
    ...
    try:
        ...
        return get_response(success=True, message='Configuration retrieved', data=config_data)
    except Exception as e:
...
def list_instances() -> tuple:
    ...
    try:
        ...
        return get_response(success=True, message=f'Retrieved {len(formatted_instances)} instances',
...
def get_instance_details(instance_id: str) -> tuple:
    ...
    try:
        ...
        return get_response(success=True, message='Instance details retrieved', data=instance_data)
    except Exception as e:
        return handle_error(e, 400)
...
def deploy_agent() -> tuple:
    ...
    try:
        ...
        if not agent_type or not instance_ids:
            return get_response(success=False, message='Missing required fields', status_code=400)
        logger.info(f"Deployment requested: agent={agent_type}, instances={instance_ids}")
        aws_cfg = config_manager.get_aws_config()
        deploy_cfg = config_manager.get_deployment_config()
        ...
        if agent_type.lower() == 'qualys':
            ...
            return get_response(success=False, message=f'Unknown agent type: {agent_type}', status_code=400)
        if dry_run:
            return get_response(success=True, message='Dry-run successful',
        ...
        return get_response(success=True, message='Deployment completed', data=result)
    except Exception as e:
        return handle_error(e, 400)
...
def get_deployments() -> tuple:
    ...
    try:
        ...
        return get_response(success=True, message='Deployments retrieved', data={'deployments': deployments})
    except Exception as e:
        return handle_error(e, 400)
...
def run_health_check() -> tuple:
    ...
    try:
        ...
        return get_response(success=True, message='Health check completed', data=results)
    except Exception as e:
        return handle_error(e, 400)
...
def not_found(error):
    """Handle 404 errors."""
    return get_response(success=False, message='Endpoint not found', status_code=404)
...
def internal_error(error):
    ...
    return get_response(success=False, message='Internal server error', status_code=500)
